...
2026-07-12 使用 5 个地址和 1,000,000 帧完成约 31.2 分钟等效负载验证：`stored=1,000,000`，写入用时 `22.220 s`，吞吐约 `45,005 帧/s`，数据库大小 `70,737,920 bytes`，约 `70.7 bytes/帧`。按工具的 535 帧/s 合成总线速率，写入吞吐约为该负载的 84 倍；新数据库未创建 `signal_samples`。
